// src/Pages/Login.jsx
import React, { useState, useEffect } from 'react'; // Added useEffect for toast timeout
...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (showLoginSuccessToast) {
      timer = setTimeout(() => {
        setShowLoginSuccessToast(false);
      }, 3000); // Toast visible for 3 seconds (total, navigation happens at 2s)
    }
    return () => clearTimeout(timer); // Cleanup timer
  }, [showLoginSuccessToast]);

  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-100 px-4 py-12 sm:px-6 lg:px-8 relative"> {/* Added relative for toast positioning */}
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6 bg-white p-8 shadow-xl rounded-xl">
          ...
          <div>
            <button
              type="submit"
              disabled={loading || showLoginSuccessToast} // Disable button while toast is shown
            ...
            >
              <span className="absolute left-0 inset-y-0 flex items-center pl-3">
                {loading || showLoginSuccessToast ? ( // Show spinner if loading or toast is visible
